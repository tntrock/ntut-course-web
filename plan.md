...
**驗收**(已完成,見附錄 D.31–D.34):

- [x] 加入兩門時段重疊的課,兩者都標紅且頂部顯示衝堂數
- [x] `time_slots` 為空的課(如班週會)出現在「未排入時段」區,不會消失
- [x] **關掉網路**重新整理,課表仍能從快照完整渲染
- [x] 手動把快照的 `time_slots` 改掉,重新載入後顯示「時段已異動」與 舊 → 新
- [x] 把課號改成不存在的值,顯示「此課已停開」且不崩潰
- [x] localStorage 內容損毀(塞入非 JSON)時走備份重置流程,不白畫面
- [x] 匯出的 JSON 能匯入回來,資料一致
...
**驗收**(已完成,見附錄 D.35–D.37):

- [x] 5 種事件型別都有對應的視覺
- [x] `bulk_change` 的 `by_department` 代碼轉成中文名(`01` → 教務處)
- [x] `course_changed` 的逐欄位 diff 正確顯示 舊 → 新
- [x] `checked_at` 超過 12 小時顯示警告
- [x] `changes.json` 為空事件時顯示「近期無異動」而非空白頁
...
**驗收**(已完成,見附錄 D.38–D.41):

- [x] 可安裝到手機主畫面,圖示與啟動畫面正確
- [x] 飛航模式下開站,搜尋過的學期仍可搜尋與瀏覽
- [x] 離線橫幅正確顯示資料時間
- [x] PNG 匯出的中文字型正常 —— **只在 Chrome 驗過**,Safari / Firefox 沒有環境(見 D.40)
- [x] Accessibility 100、Best Practices 100、SEO 100;Performance 以實測指標代替分數(見 D.41)
- [x] 全站可純鍵盤操作,搜尋 → 篩選 → 加入課表
...
- [x] push 到 main 自動部署成功
- [x] PR 產生 preview 網址
- [x] 直接開深層網址(`/course/115-1/364893`)不會 404(SPA fallback 生效)
- [x] 自訂網域 HTTPS 正常
- [ ] Analytics 有數據進來 —— **未設定**,那要在 Cloudflare dashboard 開啟,
      不是程式碼能做的事
- [x] 頁尾免責聲明可見
...
首頁一直是 Phase 0 那個「把驗收數字印出來」的暫時頁面。改成真正的入口:

- **兩張動線卡**:搜尋、瀏覽。描述用的是實際數字(`60 個系所、293 個班級`),
  不是空話
- **校級單位快捷** —— 通識、體育這些**每個人都要修**的課在這裡。這一組不是我挑的,
  是 `departments.json` 裡 `college` 為 `null` 的那一組(見 D.18),學校改了組成
  首頁跟著變
- 本學期概況與資料更新時間

首頁**順手預取 `departments.json`**(gzip 約 5 KB):它自己要用,而搜尋頁與瀏覽頁
需要同一份 —— 等於把下一頁的快取暖起來,不是多花的請求。

原本第四格放 `merged_course_count`(合開課數)。「其中合開 309」在首頁是個沒有
上下文就看不懂的數字,換成收錄學期數(51)—— 那是這個站真正的特點,而且看得懂。

### D.30 課程詳情頁:補上學期、徽章分級、寬螢幕兩欄

- **學期一定要看得見。** 原本標題下只有課號。分享連結進來的人不知道自己看的是哪
  一學期,而課號跨學期不通用(§1.3.7)—— 一門 114-2 的課看起來會跟本學期的
  一模一樣。現在是 `115-1 · 364893`
- 徽章改用與搜尋卡片**共用的元件**(`components/ui/Badge`)。兩邊各寫一份的話,
  同一個欄位遲早在兩個地方長得不一樣
- 備註加上「備註」標籤與左側色條:不標的話看起來像本站的提示,實際上那是學校寫的
- 資訊列在寬螢幕排**兩欄**,六個短欄位擠成三列,少捲一半。內容長的欄位
  (學程、隨班附讀、實習)佔滿整列,不然會被擠到換行

### D.31 儲存層的兩條硬規則

個人資料是全站唯一會寫入的東西,所以 `lib/storage.ts` 訂了兩條:

1. **`loadStore()` 永遠不丟例外、永遠回傳可用的結構。** 讀不到、內容壞掉、版本
   不對都一樣 —— 個人資料出問題不該讓整個網站變成白畫面。
2. **絕不靜默丟掉。** 整包救不回來時先原樣備份再重置。

實作時發現「備份後重置」我一開始只做了一半:備份寫了,但**壞掉的內容還留在主
key**。那會讓每次載入都重新備份一次,而且使用者永遠停在壞掉的狀態。「重置」要
真的把預設值寫回去。

還有一個更嚴重的:**課表默默變空而使用者完全不知道發生什麼事**。加了
`RecoveryNotice`,明講「讀不出來、已重置、原始內容可以下載」。

這個提示第一版沒出現,原因是**順序**:備份是在 `loadStore()` 裡寫的,而那要等第
一次 `useStore()` 才發生 —— 元件先讀備份就永遠讀到 `null`。修法是在讀之前先呼叫
`useStore()`,並把這個相依關係寫進註解。

損毀之外的三種情況也都有對應:個別欄位壞掉只重置那一塊(課表還在就該留著)、
課表裡混進非課程只濾掉那一筆、`localStorage` 完全不可用(無痕視窗)時整站照常
運作只是存不下。寫入失敗會**回報原因**而不是假裝存好了 —— 使用者以為加了課、
重新整理後不見,比當下就說「存不下」更糟。

### D.32 課表的三個「看起來對其實錯」的地方

- **學分為 `null` 的課不能當成 0。** 當成 0 會讓總學分看起來是對的,其實少算。
  改成不計入總分但**數出來**:「N 門課沒有提供學分數,未計入總學分」。
- **`required` 是三態。** 把 `null` 併進選修會讓學分試算悄悄算錯,所以必修 /
  選修 / 未標示分三欄,而且未標示的那一欄會另外說明。
- **每日課量算節數不是門數。** 「週一 3 門」和「週一 9 節」對排課的人是不同的
  資訊。

最早 / 最晚一堂一律以 `meta.periods` 的順序判斷 —— 字典序會說夜間的 `A` 比 `5` 早。

### D.33 衝堂的課要並排,不能疊在一起

第一版把三門同時段的課放在同一個網格區域,結果課名互相蓋掉完全看不懂。使用者
看得到「有衝堂」卻看不出**是哪兩門在撞**,而那正是他最需要知道的事。

`layoutRuns` 因此多回傳 `lane` / `lanes`,用寬度與左邊距把一欄切成幾份。分欄以
**互相連通的一群**為單位而不是整天:早上衝堂不該讓下午那門獨立的課也變成半格寬。

同一件事的另一面是**跨列合併**:一門三節的課要合成一塊,不然課名會印三次;
但不連續的節次不能連成一塊,否則等於在課表上宣稱中間那節也要上課 ——
與 D.9 的 `2、4` 不能寫成 `2-4` 是同一個問題。

### D.34 課表不等索引,但索引還是會到

`/schedule` 的 loader **只等 `meta`,不等學期索引**:課表整張都畫得出來只靠
localStorage 的快照,飛航模式重新整理也要看得到。索引是用來比對異動的,晚一點到
就晚一點標。

實測離線重新整理時,課表、未排入時段、統計全部正常渲染 —— 而且**異動偵測也還在**,
因為索引從 Cache Storage 命中了(§2.2 的版本化快取層)。「離線就沒有異動偵測」
只會發生在從沒載入過索引的裝置上。

### D.35 §3.6 有兩處與實測不符

`changes.json` 實測(2026-09-06,15 筆事件):

1. **沒有 `teacher_added` / `teacher_removed`。** §3.6 的表格列了這兩種,但
   `ChangeEventType` 沒有,實測資料裡也沒有。實際出現的是 `baseline`、
   `bulk_change`、`course_removed`、`course_changed` 四種,加上只出現在
   `bulk_change.samples` 裡的 `course_added` —— 五種視覺都做了。
2. **不是「只有最新兩個學期會產生事件」。** `baseline` 橫跨 **7 個學期**
   (112-1 到 115-1)。真正的異動事件確實只有 115-1,但 baseline 不是異動。

這個差別有實際後果:代碼轉中文名要用**逐學期**的 `departments.json` /
`classes.json`。不篩的話 7 個學期就是 14 個請求。`semestersNeedingNames()`
因此排除 `baseline`(它只有課程數,沒有任何代碼)—— 實測只剩 1 個學期要載。

### D.36 時間軸要用台北日期分組

事件的 `at` 是 UTC。直接切 ISO 字串的前 10 個字會把 UTC 傍晚的事件分到前一天,
但台北時間已經是隔天 —— 時間軸依日期分組,分錯天等於資訊是錯的。
`taipeiDate()` 用 `en-CA` 的短日期格式(剛好就是 `YYYY-MM-DD`)換算。

同一天內也照時間由新到舊排,順序才不會跟著來源檔的排列跳動。

### D.37 三個「要講清楚」的地方

- **時間是本站偵測到的,不是學校異動的。** 實際異動落在前一次抓取與這次之間,
  頁面上與每個時間的 `title` 都標註了。
- **`checked_at` 是今天但沒有事件 = 學校真的沒動;停在幾天前 = 爬蟲沒在跑。**
  空狀態的文案會依 `isStale` 分成兩種說法,不然使用者會把後者當成前者。
- **`bulk_change` 的橫條圖不只是好看。** 集中在少數幾個單位 = 學校開了一批課;
  散落在幾十個系所 = 可能是解析器出問題。這個判讀方式直接寫在卡片上。

差異的值也一律翻成中文:`department_ids` 從 `["31"]` 變 `["B2","31"]` 對使用者
毫無意義,翻成「電機系 → 半導體學士學位學程、電機系」才看得懂。空值顯示成
「無」而不是空字串 —— 空字串會讓「舊 → 新」渲染成「 → 電子系」,看起來像壞掉。

#### 實測發現:重複的 baseline

112-1、112-2、113-1、113-2、114-1 各有**兩筆一模一樣的 `baseline`**
(課程數相同,相隔約 45 分鐘)。這看起來是 crawler 端重複建立基準線。

**沒有在前端去重** —— 那會把一個 crawler 應該知道的訊號藏起來。

### D.38 主題設定原本沒有人在用

`settings.theme` 從 Phase 4 就存在 store 裡,`index.html` 的首屏腳本也讀它 ——
**但沒有任何 UI 可以改,執行期也沒有人套用**。所以在 Phase 6 之前,深色模式其實
只是跟隨系統,那個設定欄位是死的。

補上切換器之後有兩個細節:

- **同時設 `color-scheme`。** 只切 `dark` class 的話,原生捲軸與 `<select>` 的
  下拉在深色底下仍然是白的。
- **跟隨系統時要監聽 `prefers-color-scheme` 的變化。** 使用者在作業系統切換外觀
  應該即時反應,不必重新整理。

### D.39 Service worker 只管 app shell

`registerType: 'prompt'` 而不是 `autoUpdate`:自動更新會在背景換掉程式碼並重新
載入頁面 —— 使用者可能正排課排到一半。什麼時候更新由他決定。

Workbox 的 `globPatterns` **只涵蓋 app shell**(HTML / JS / CSS / 圖示),
不碰 API 資料。那一份由 §2.2 的版本化 Cache Storage 管:兩套快取管同一份資料
只會互相打架,而且只有那一層知道 `generated_at` 版本號,Workbox 不知道。

圖示是新做的(原本 repo 裡是 Vite 的預設 logo),與頁首那顆「北」徽章同色同字。
`maskable` 那一版把字縮到中央安全區,被裁成圓形時才不會切到。

### D.40 PNG 匯出只在 Chrome 驗過

§4 的驗收寫「Chrome / Safari / Firefox 各測一次」。**我只有 Chrome 的環境**,
另外兩個沒有驗證,這裡如實記下。

已經做的降低風險:

- **不用 webfont。** 全站中文本來就走系統字型堆疊,字型嵌不進去會讓中文整片變成
  豆腐,那是 §3.5 就標記的風險。
- **匯出版面用寫死的十六進位色,不吃站上的 CSS 變數與 `oklch()`。** 截圖工具是把
  computed style 塞進 SVG,顏色函式在那條路徑上是額外的風險。順帶一提,分享出去
  的課表本來就是淺色比較好讀、好印。
- 匯出失敗時顯示「可以改用瀏覽器截圖」,而不是靜靜地什麼都沒發生。

Chrome 實測:1200px 版面輸出 2400×1824 的 PNG(2 倍解析度),中文正常。

### D.41 CLS 0.15 全部來自頁尾

Lighthouse 的 Accessibility / Best Practices / SEO 都是 **100**,但這個工具**不含
Performance 分數**,所以改為實測 Web Vitals。

第一次量到 **CLS 0.1525** —— 超過 0.1 的門檻。追下去發現是**單一次位移,來源是頁尾**:
載入狀態只有一行「載入中…」,頁尾因此停在視窗中間;內容一到,頁尾被推到畫面外,
被算成一次大幅位移。

修法是讓載入狀態撐滿約一個視窗高(`min-h-[80dvh]`),頁尾一開始就在視窗外。
修好後:
...
另外兩個 Lighthouse 抓到的真問題:

- **手機版 logo 連結沒有可讀名稱** —— 站名在小螢幕是 `hidden sm:inline`,只剩
  `aria-hidden` 的徽章,連結因此變成沒有名字。補上 `aria-label`。
- **`/robots.txt` 被 SPA fallback 回成 HTML**。加了真的 `robots.txt`。

鍵盤動線實測:37 個可見的可聚焦元素全部聚焦得到,Tab 順序是
logo → 搜尋框 → 導覽 → 外觀 → 學期 → 篩選 → 各篩選分區 → 結果(連結與加入按鈕交錯),
聚焦到「加入課表」按 Enter 確實加進課表且 `aria-pressed` 翻成 `true`。

### D.42 關於頁不重寫任何一份既有文案

§3.8 特別交代「完整免責聲明**讀 `meta.disclaimer`,不要自己重寫一份**」。同樣的
道理套用到另外兩處:

- **API base URL 從 `lib/api.ts` 匯出**,不在關於頁寫死第二份。環境變數改了卻
  忘了改文案的話,使用者照著頁面上的網址是打不到東西的。
- **端點清單直接渲染 `meta.endpoints`**,crawler 加端點時關於頁自動跟上。

已知限制那一段則是**摘要 crawler README 的「資料範圍與已知限制」**,挑對使用者
真正有影響的幾條:只涵蓋上課時間表、課號跨學期不通用且合開課是不同課號、
修課人數是抓取當下的值、大綱的涵蓋範圍(**用算的,見 D.43**)、沒有英文課名、
沒有進修部、舊學期系所代碼對不起來。

另外補上一條 crawler README 沒有、但只有本站需要說的:**個人資料只存在這台裝置**。

#### JSX 裡的 `**粗體**` 不會被解析

寫關於頁時我又在 JSX 裡打了 markdown 的 `**...**`,那會原樣印在畫面上 ——
Phase 5 的異動頁犯過同一個錯。已全部換成 `<strong>`。

### D.43 大綱的涵蓋範圍改成算出來的

§1.3.5 當初實測「**只有 115-1 有大綱**,114-2 與 110-1 全部 404」,關於頁因此寫了
一句「教學大綱只涵蓋 110-1 以後」。

2026-09-06 重新核對線上資料,那句話已經兩頭都錯了:

|        | 寫死的說法     | 實際                      |
| ------ | -------------- | ------------------------- |
| 最舊   | 110-1          | **110-2**(110-1 還沒補到) |
| 學期數 | 「110-1 以後」 | **10 個**,115-1 到 110-2  |
| 份數   | 未提           | **21,136**                |

爬蟲正在**往回逐期補**(110-2 於 06:34 凍結、111-1 於 05:41),所以任何寫死的
範圍都會在某一天悄悄變成錯的 —— 而且它讀起來仍然很合理,不會有人發現。

改成 `syllabusCoverage(progress, order)` 由 `syllabus.json` 算出來。兩個來源都要
看:**更新中的學期在 `semesters[]`,抓完的搬到 `frozen`**,只看其中一邊會漏掉
九成的學期。排序用 `meta.semesters` 的順序而不是字串排序 —— 字串排序碰到
`99-1` 那種舊學期會排錯。

同時確認 D.28 那套 frozen 的處理在新資料下仍然正確:線上實測 `114-1/352146`
取到 `?v=2026-09-05T11:32:57Z`(= `frozen.at`)、200,同學期沒有 `syllabus_url`
的 `353177` 連分頁都不顯示,**沒有任何一發 404**。

### D.44 `stage` 是「階段」不是「年級」

使用者回報課程資訊的年級不對，「有些課根本不是那個年級的課」。查證屬實，
而且錯得很徹底 —— 站上把 `stage` 標成「年級」，但它根本不是年級。

學校原始表格的欄位順序是：

```
課號 | 課程名稱 | 階段 | 學分 | 時數 | 修 | 教師 | 日 一 二 三 四 五 六 | 教室 | 人 | 撤 | …
```

`stage` 對應的是**「階段」**。年級在「班級」那一欄（資工四）。
...
| 班級是「資工四」而 `stage` 為 `"1"` | **291 門全部**      |
| 班級含「一」而 `stage` 為 `"3"`     | 6 門                |

所以「數位影像處理」（備註寫明「資工四和資工所合開」）在站上顯示成
**「1 年級」**。這不是邊緣案例，是每一門四年級的課都在說謊。

各系所的填法還不一致：體育專項（七）~（十一）全是 `3`；智動的國文在二/三/四
年級分別是 `3`/`5`/`7`。**所以不猜語意** —— 改用學校自己的欄位名，學校填什麼
就顯示什麼。

`"1"` 不顯示：96% 的課都是這個值，擺在每一頁只是雜訊。要對照的話課程頁本來
就有「學校原始頁面」的連結。

三處要一起改，漏掉任何一處就會留下同一個錯誤：

1. `routes/course.$semester.$courseId.tsx` 的徽章
2. `lib/changes.ts` 的 `FIELD_LABELS`（異動頁也把 `stage` 標成「年級」）
3. `types/api.ts` 的欄位註解
