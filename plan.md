...
- [ ] 有大綱的課(如 `364893`)完整顯示 outline / schedule / assessment / SDGs
- [ ] `syllabus_url` 為 `null` 的課(808 門之一)**不顯示大綱分頁**,且不發出 404 請求
- [ ] `has_content: false` 的課顯示「教師尚未填寫」而非空白
- [ ] 切到 `114-2` 的課,大綱區顯示「本學期未收錄」
- [ ] 直接開 `/course/115-1/364893`(冷啟動)能正確載入
- [ ] `notes` 的合開資訊有顯示
...
- [ ] `college` 為 `null` 的 5 個單位歸在「校級單位」,不顯示 null
- [ ] 學院級單位(`C0` `C2` `C5` `C7`)有標註說明
- [ ] 同名教師的兩個代碼是**兩個獨立頁面**,課不會混在一起
- [ ] 學程名含特殊字元時路由正常(URL encode)
- [ ] 從課程詳情點教室,能看到該教室的所有課
